...
"""
    Basic example code is taken from Bluesky examples and then adapted to current project :)
"""
...
def upload_images(
    pds_url: str, access_token: str, image_paths: List[str], alt_texts: List[str]
) -> Dict:
    images = []
    for img_alt, img_path in zip(alt_texts, image_paths):
        ...
        blob = upload_file(pds_url, access_token, img_path, img_bytes)
        images.append({"alt": img_alt or "", "image": blob})
    return {
...
def create_post(args):
    ...
    if args.image:
        post["embed"] = upload_images(args.pds_url, session["accessJwt"], args.image, args.alt_text)
    elif args.embed_url:
        post["embed"] = fetch_embed_url_card(args.pds_url, session["accessJwt"], args.embed_url)
    elif args.embed_ref:
